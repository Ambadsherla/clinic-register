...
<!-- Setup — shown only on very first use -->
...
<script>
/* ══════════════════════════════════════════════════
   CLINIC REGISTER — PostgreSQL persistent storage
   Data survives server restarts, redeploys, forever
══════════════════════════════════════════════════ */
...
async function submitSetup() {
  ...
  try {
    ...
    render();
    focusInput();
    toast('ODIP starts from ' + val + ' — all set!', 'ok');
...
async function savePatient() {
  ...
  sheetId: state.sheets[activeSheet].id
    ...
    // Add to last sheet in local state
    state.sheets[state.sheets.length - 1].patients.push(data.patient);
...
async function undoLast() {
  try {
    const res  = await fetch('/api/patient/last', { method: 'DELETE' });
    const data = await res.json();
    if (!res.ok) throw new Error(data.error);
    state.sheets[state.sheets.length - 1].patients.pop();
    state.nextOdip = data.nextOdip;
    activeSheet    = state.sheets.length - 1;
    render();
...
async function deletePatient(patientId, patientName) {
  ...
  try {
    ...
    // Reload full state — ODIP renumbering happened on server
...
async function createSheet() {
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok) throw new Error(data.error);
    state.sheets.push({ id: data.sheetId, name: data.sheetName, patients: [] });
    activeSheet = state.sheets.length - 1;
    render();
    focusInput();
